...
class User(Bank):
    ...
    def deposite(self, amount):
        if amount > 0:
            ...
            self.transactionHistory = transactionStatus
            self.bank.totalBalance += amount
            return f"successfully deposited {amount} in your account"
        else:
    ...
    def widraw(self, amount):
        if amount <= self.bank.totalBalance:
            if amount > 0 and amount <= self.balance:
                ...
                return f"widraw successful you got {amount}"
            else:
                if amount < 0:
...
print(dbbluser1.transactionHistory)
